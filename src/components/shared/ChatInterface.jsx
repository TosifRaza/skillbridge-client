...
const ChatInterface = ({ conversationId, otherUserEmail, currentUserEmail }) => {
  const dispatch = useAppDispatch();
  const socket = useSocket();
  const { messages, loading } = useAppSelector((state) => state.chat);
  const { user } = useAppSelector((state) => state.auth);
...
// }, [dispatch, conversationId, socket]);
  ...
  const handleSendMessage = (e) => {
    ...
    const messageData = {
      ...
      text: messageInput,
      imageUrl: null,
      senderEmail: currentUserEmail || user?.email, // ADD SENDER EMAIL
      recipientEmail: otherUserEmail // ADD RECIPIENT EMAIL
    };
  ...
  const handleTyping = (e) => {
    setMessageInput(e.target.value);
    if (socket) {
      socket.emit('typing', { conversationId, senderEmail: currentUserEmail || user?.email });
    }
  };
    ...
    try {
      ...
      if (url && socket) {
        const messageData = {
          ...
          text: '',
          imageUrl: url,
          senderEmail: currentUserEmail || user?.email,
