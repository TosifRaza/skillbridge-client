...
import { HiOutlineLogout, HiOutlineHome, HiOutlineUsers, HiOutlineBriefcase, HiOutlineShieldCheck, HiOutlineExclamationCircle, HiOutlineTrendingUp} from 'react-icons/hi';
...
const AdminLayout = () => {
  ...
  const navItems = [
    ...
    { icon: HiOutlineShieldCheck, label: 'Verifications', path: '/admin/verifications' },
    { icon: HiOutlineBriefcase, label: 'Jobs', path: '/admin/jobs' },
    { icon: HiOutlineExclamationCircle, label: 'Reports', path: '/admin/reports' },
      ...
      <aside className="w-64 bg-surface-900 dark:bg-black text-white flex flex-col fixed h-full z-40">
        <div className="h-16 flex items-center px-6 border-b border-surface-800">
          <Link to="/admin" className="flex items-center gap-2">
            <div className="w-8 h-8 bg-red-500 rounded-lg flex items-center justify-center text-white font-bold text-sm">A</div>
        ...
        <header className="h-16 bg-white dark:bg-surface-800 border-b border-surface-200 dark:border-surface-700 sticky top-0 z-30 flex items-center justify-between px-8 transition-colors">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              <span className="text-sm font-medium text-surface-700 dark:text-surface-300 hidden md:block">Admin: {user?.email}</span>
            </div>
          </div>
        </header>
